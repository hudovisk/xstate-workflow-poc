...
describe("receive-email-confirmed-myaccount", () => {
  it("should send email event to BR", async () => {
    ...
    expect(eventSender).to.be.calledWith("AdobeEmailMyAccountEvtBR");
  });
  ...
  it("should send email event to MX", async () => {
    ...
    await expect(promise).to.be.fulfilled;
    expect(eventSender).to.be.calledOnce;
    expect(eventSender).to.be.calledWith("AdobeEmailMyAccountEvtMX");
  });
});
